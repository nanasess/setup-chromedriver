...
function getDownloadUrl(versionInput, chromeapp) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let result;
        if (plat === 'win32') {
            // use PowerShell to invoke Chrome executable path with spaces
            result = yield exec.getExecOutput('powershell', [
                '-NoProfile',
                '-Command',
                `& '${chromeCmd.replace(/'/g, "''")}' --version`
            ]);
        }
        else {
            // use POSIX shell on macOS/Linux to invoke path containing spaces
            const cmdStr = `"${chromeCmd}" --version`;
            result = yield exec.getExecOutput('/bin/sh', ['-c', cmdStr]);
        }
