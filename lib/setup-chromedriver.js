...
const unix_1 = require("./installer/unix");
const windows_1 = require("./installer/windows");
async function run() {
    try {
        ...
        if (arch == "win32") {
            await (0, windows_1.installOnWindows)({ version, chromeapp });
        }
        else {
            await (0, unix_1.installOnUnix)({ version, arch, chromeapp });
        }
    }
    catch (error) {
