...
import { buildLegacyLatestReleaseUrl, extractDriverUrlFromJson, findFallbackVersion, } from "../chromedriver-helper.js";
 ...
 */
export async function detectFullChromeVersion(platform, chromeapp) {
    let stdout = "";
    const listeners = {
        stdout: (data) => {
 ...
 */
export async function resolveModernDownload(version, arch) {
 ...
 */
export async function resolveLegacyVersion(majorVersion) {
